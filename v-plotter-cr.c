...
#define LEFT_STEPPER_DIR  17
#define LEFT_STEPPER_STEP 4

#define RIGHT_STEPPER_DIR  22
#define RIGHT_STEPPER_STEP  23
...
//Lengths given in millimiters //sup_left
#define BASELENGTH              820
#define CORDLENGTH_LEFT         340
#define CORDLENGTH_RIGHT        580
...
//Steps per mm (cr) 200/(8*PI)=7,957747155
double StepsPermm = 800.0 / (8.0*PI);
...
void MakeStepLeft(int direction){
  digitalWrite(LEFT_STEPPER_DIR, direction);
  digitalWrite(LEFT_STEPPER_STEP,1);
  usleep(STEP_PAUSE);
  digitalWrite(LEFT_STEPPER_STEP,0);
  usleep(STEP_PAUSE);
}

//++++++++++++++++++++++++++++++ MakeStepRight ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void MakeStepRight(int direction){
  digitalWrite(RIGHT_STEPPER_DIR,direction);
  digitalWrite(RIGHT_STEPPER_STEP,1);
  usleep(STEP_PAUSE);
  digitalWrite(RIGHT_STEPPER_STEP,0);
  usleep(STEP_PAUSE);
}
